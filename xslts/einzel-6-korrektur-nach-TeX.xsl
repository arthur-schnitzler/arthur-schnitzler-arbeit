...
-\->      <xsl:text>{\\[\baselineskip]}</xsl:text>
   ...
   <!-- Großbuchstaben -->
   <xsl:template
      match="hi[@rend = 'capitals' and not(descendant::note) and not(descendant::footNote)]//text()">
      <xsl:value-of select="upper-case(.)"/>
   </xsl:template>
   <xsl:template
      match="hi[(@rend = 'capitals' and descendant::note) or (@rend = 'capitals' and descendant::footNote)]//text()">
      <xsl:choose>
         <xsl:when
