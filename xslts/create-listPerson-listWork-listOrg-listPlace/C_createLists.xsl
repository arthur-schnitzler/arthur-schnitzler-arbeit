...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns="http://www.tei-c.org/ns/1.0"
    ...
    <xsl:template match="tei:item/@n" mode="authors">
        ...
        <xsl:choose>
            <xsl:when test="doc-available($eintrag)">
                <xsl:for-each select="document($eintrag)//author/@*[name()='ref' or name()='key']">
                    <xsl:call-template name="authorNr">
                        <xsl:with-param name="nummer" select="replace(., 'person__', '')" />
                    </xsl:call-template>
                </xsl:for-each>
            </xsl:when>
            <xsl:otherwise>
    ...
    <xsl:template match="tei:listPerson/tei:person[@n]">
        ...
        <xsl:if test="not(preceding-sibling::*)"><!-- hier noch
        einmal die Autorinnen und Autoren behandeln. Achtung, es entstehen Dubletten-->
            <xsl:apply-templates select="ancestor::tei:body/tei:list/tei:item/@n" mode="authors"/>
        </xsl:if>
    </xsl:template>
