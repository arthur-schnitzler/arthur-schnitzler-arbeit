...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
   ...
   <!-- Globale Parameter -->
   <xsl:param name="persons" select="document('listperson.xml')"/>
   <xsl:param name="works" select="document('listwork.xml')"/>
   <xsl:param name="orgs" select="document('listorg.xml')"/>
   <xsl:param name="places" select="document('listplace.xml')"/>
   ...
   <xsl:function name="foo:person-fuer-index">
      ...
      <xsl:variable name="kTodesort" as="xs:string?">
         <xsl:choose>
            <xsl:when test="$indexkey/tei:death/tei:placeName[not(@type)]">
               <xsl:value-of
                  select="fn:normalize-space($indexkey/tei:death/tei:placeName[not(@type)])"/>
            </xsl:when>
            <xsl:when test="$indexkey/tei:death/tei:placeName[@type = 'deportation']">
               <xsl:value-of
                  select="concat('deportiert ', fn:normalize-space($indexkey/tei:death/tei:placeName))"
               />
            </xsl:when>
            <xsl:when test="$indexkey/tei:death/tei:placeName[@type = 'burial']">
               <xsl:value-of
                  select="concat('beerdigt ', fn:normalize-space($indexkey/tei:death/tei:placeName))"
               />
            </xsl:when>
         </xsl:choose>
      </xsl:variable>
      <xsl:variable name="kGeburtsort" as="xs:string?" select="$indexkey/tei:birth/tei:placeName"/>
      ...
      <xsl:choose>
         <xsl:when test="normalize-space($kName) = '??'">
            <xsl:choose>
         ...
         <xsl:otherwise>
            <xsl:text>\textcolor{red}{XXXX INDEXFEHLER}</xsl:text>
         </xsl:otherwise>
      </xsl:choose>
      ...
      <xsl:if test="(not($kBeruf = '') or empty($kBeruf)) and not($kName = '??')">
         <xsl:text>, \emph{</xsl:text>
         <xsl:value-of select="normalize-space($kBeruf)"/>
         <xsl:text>}</xsl:text>
         <xsl:text/>
      </xsl:if>
   </xsl:function>
   ...
   <xsl:function name="foo:werk-in-index">
      ...
      <xsl:choose>
         ...
         <xsl:when test="($work-entry/tei:author/tei:surname = '') and $im-text">
            <xsl:text>\pwindex{</xsl:text>
         </xsl:when>
         <xsl:when test="($work-entry/tei:author/tei:surname = '') and not($im-text)">
            <xsl:text>\pwindex{</xsl:text>
         </xsl:when>
         <xsl:when test="not($work-entry/tei:author/tei:surname) and $im-text">
            <xsl:text>\pwindex{</xsl:text>
         </xsl:when>
         <xsl:when test="not($work-entry/tei:author/tei:surname) and not($im-text)">
            <xsl:text>\pwindex{</xsl:text>
         </xsl:when>
         <xsl:otherwise>
            <xsl:value-of select="foo:person-in-index($work-entry/tei:author/tei:surname, $im-text)"/>
            <xsl:text>!</xsl:text>
         </xsl:otherwise>
      </xsl:choose>
   ...
   <xsl:function name="foo:org-in-index">
      ...
      <xsl:choose>
         <xsl:when test="string-length($org-entry/tei:orgName[1]) = 0">
            <xsl:text>\orgindex{XXXX ORGangabe fehlt</xsl:text>
         </xsl:when>
         <xsl:otherwise>
            <xsl:choose>
               <xsl:when test="$first != ''">
   ...
   <xsl:template match="TEI[starts-with(@id, 'E')]">
      <root>
         ...
         <xsl:choose>
            <xsl:when
               ...
               <xsl:value-of
                  select="count(descendant::revisionDesc/change[contains(text(), 'Index check')])"/>
   ...
   <xsl:template match="TEI">
      ...
      <xsl:if test="(@bw != preceding-sibling::TEI[1]/@bw) or position() = 1">
         <xsl:text>\addpart[</xsl:text>
         <xsl:value-of
            select="concat(key('person-lookup', (@bw), $persons)/forename, ' ', key('person-lookup', (@bw), $persons)/tei:persName/tei:surname)"/>
         <xsl:text> (</xsl:text>
         <xsl:value-of select="count(following-sibling::TEI[@bw = current()/@bw]) + 1"/>
         <xsl:text> \textsuperscript{Von AS </xsl:text>
         <xsl:value-of
            select="count(parent::root/TEI[@bw = current()/@bw and descendant::correspDesc/correspAction[@type = 'sent']/persName/@ref = '#2121'])"/>
         <xsl:text>}</xsl:text>
         <xsl:text>)]{</xsl:text>
         <xsl:value-of
            select="concat(key('person-lookup', (@bw), $persons)/forename, ' ', key('person-lookup', (@bw), $persons)/tei:persName/tei:surname)"/>
      ...
      <xsl:variable name="correspAction-date"
         select="descendant::correspDesc/correspAction[@type = 'sent']/date" as="node()"/>
      <xsl:variable name="dokument-id">
         <xsl:choose>
            <xsl:when test="descendant::correspDesc">
...
-->
   ...
   <xsl:function name="foo:sender-empfaenger-in-personenindex">
      ...
      <xsl:choose>
         ...
         <xsl:when test="not($sender-key = '#2121')">
   ...
   <xsl:template match="incident/desc">
      <xsl:variable name="typ" select="parent::incident/@type"/>
      <xsl:variable name="anzahl" select="count(ancestor::additions/child::incident[@type = $typ])"/>
      <xsl:variable name="posizion"
         select="count(parent::incident/preceding-sibling::incident[@type = $typ])"/>
      <xsl:variable name="art" as="xs:string">
         <xsl:choose>
            <xsl:when test="$typ = 'supplement' and $anzahl = 1">
               <xsl:text>Beilage</xsl:text>
            </xsl:when>
            <xsl:when test="$typ = 'supplement'">
               <xsl:text>Beilagen</xsl:text>
            </xsl:when>
            <xsl:when test="$typ = 'postal'">
               <xsl:text>Versand</xsl:text>
            </xsl:when>
            <xsl:when test="$typ = 'receiver'">
               <xsl:value-of
                  select="substring-before(ancestor::teiHeader/descendant::correspDesc/correspAction[@type = 'received']/persName[1], ',')"
               />
            </xsl:when>
            <xsl:when test="$typ = 'archival'">
               <xsl:text>Ordnung</xsl:text>
            </xsl:when>
            <xsl:when test="$typ = 'additional-information'">
               <xsl:text>Zusatz</xsl:text>
            </xsl:when>
            <xsl:when test="$typ = 'editorial' and $anzahl = 1">
               <xsl:text>Editorischer Hinweis</xsl:text>
            </xsl:when>
            <xsl:when test="$typ = 'editorial'">
               <xsl:text>Editorische Hinweise</xsl:text>
            </xsl:when>
            <xsl:otherwise>
      ...
      <xsl:choose>
         ...
         <xsl:otherwise>
            ...
            <xsl:value-of select="$posizion + 1"/>
            <xsl:text>)&#160;</xsl:text>
            <xsl:apply-templates/>
         </xsl:otherwise>
      </xsl:choose>
   ...
   <xsl:template match="handDesc">
      <xsl:choose>
         ...
         <xsl:when test="not(child::handNote[2]) and not(handNote/@corresp)">
...
\newline{}</xsl:text>
   ...
   <xsl:template match="stamp/placeName | addSpan | stamp/date | stamp/time">
      <xsl:if test="current() != ''">
         <xsl:choose>
            <xsl:when test="self::placeName and @ref = 'A000250'"/>
            <!-- Wien raus -->
            ...
            <xsl:when test="self::placeName">
               <xsl:value-of
                  select="foo:indexName-Routine('place', false(), tokenize(@ref, ' ')[1], substring-after(@ref, ' '), false(), false(), false())"
               />
            </xsl:when>
         </xsl:choose>
         <xsl:choose>
   ...
   <xsl:function name="foo:briefsender-mehrere-persName-rekursiv">
      ...
      <!-- <xsl:if test="not($briefsender/persName[$briefsender-anzahl]/@ref = '#2121' and $briefempfaenger/persName[1]/@ref='#10815')">
   ...
   <xsl:function name="foo:monogr-angabe">
      ...
      <xsl:if test="$monogr/editor[1]">
         ...
         <xsl:choose>
            ...
            <xsl:when
               test="contains($monogr/editor, 'Hg.') or contains($monogr/editor, 'Hrsg.') or contains($monogr/editor, 'erausge') or contains($monogr/editor, 'Edited')">
               <xsl:value-of select="$monogr/editor"/>
            </xsl:when>
            <xsl:otherwise>
               <xsl:text>Hg. </xsl:text>
               <xsl:choose>
                  <xsl:when test="contains($monogr/editor, ', ')">
                     <xsl:value-of
                        select="concat(substring-after(normalize-space($monogr/editor), ', '), ' ', substring-before(normalize-space($monogr/editor), ', '))"
                     />
                  </xsl:when>
                  <xsl:otherwise>
                     <xsl:value-of select="$monogr/editor"/>
         ...
         <xsl:if test="count($monogr/editor/persName/@ref) &gt; 0">
            <xsl:for-each select="$monogr/editor/persName/@ref">
               <xsl:value-of select="foo:person-in-index($monogr/editor/persName/@ref, false())"/>
   ...
   <xsl:function name="foo:weitere-drucke">
      ...
      <xsl:variable name="seitenangabe" as="xs:string?"
         select="$gedruckte-quellen/biblStruct[$drucke-zaehler]//biblScope[1][@unit = 'pp']"/>
      ...
      <xsl:choose>
         <xsl:when test="$gedruckte-quellen/biblStruct[$drucke-zaehler]/@corresp">
            <xsl:if
               ...
               <xsl:value-of
                  select="foo:werk-in-index($gedruckte-quellen/biblStruct[$drucke-zaehler]/monogr/title[@level = 'm']/@ref, false(), false())"/>
         ...
         <xsl:otherwise>
            <xsl:choose>
               <xsl:when test="$drucke-zaehler = 1">
                  <xsl:value-of
                     select="foo:bibliographische-angabe($gedruckte-quellen/biblStruct[$drucke-zaehler], true(), false())"
                  />
               </xsl:when>
               <xsl:otherwise>
                  <xsl:choose>
                     <xsl:when
                        ...
                        <xsl:value-of
                           select="foo:bibliographische-angabe($gedruckte-quellen/biblStruct[$drucke-zaehler], false(), false())"
                        />
                     </xsl:when>
                     <xsl:otherwise>
                        <xsl:value-of
         ...
         <xsl:when
            ...
            <xsl:choose>
               <xsl:when test="$gedruckte-quellen/biblStruct[1]/@corresp">
                  ...
                  <xsl:if test="$gedruckte-quellen/biblStruct[1]/monogr/title[@level = 'm']/@ref">
                     <xsl:value-of
                        select="foo:werk-in-index($gedruckte-quellen/biblStruct[1]/monogr/title[@level = 'm']/@ref, false(), false())"/>
                     <xsl:text>|pw</xsl:text>
                     <xsl:text>}</xsl:text>
                  </xsl:if>
                  <xsl:choose>
   ...
   <xsl:function name="foo:bibliographische-angabe">
      ...
      <xsl:param name="onalytic" as="xs:boolean"/>
      <!-- Wenn mehrere Abdrucke und da der analytic-Teil gleich, dann braucht der nicht wiederholt werden -->
      <xsl:param name="kommentar-oder-hrsg" as="xs:boolean"/>
      <!-- Zuerst das in den Index schreiben von Autor, Zeitschrift etc. -->
      <xsl:if test="starts-with($biblstruct/analytic[1]/title[1]/@ref, 'A02')">
         <xsl:value-of
            select="foo:indexName-Routine('work', false(), tokenize($biblstruct/analytic[1]/title[1]/@ref, ' ')[1], substring-after($biblstruct/analytic[1]/title[1]/@ref, ' '), false(), false(), $kommentar-oder-hrsg)"
         />
      </xsl:if>
      <xsl:if test="starts-with($biblstruct//monogr[1]/title[1]/@ref, 'A08')">
         <xsl:value-of
            select="foo:indexName-Routine('org', false(), tokenize($biblstruct/monogr[1]/title[1]/@ref, ' ')[1], substring-after($biblstruct/monogr[1]/title[1]/@ref, ' '), false(), false(), $kommentar-oder-hrsg)"
         />
      </xsl:if>
      <xsl:if test="starts-with($biblstruct//monogr[1]/title[1]/@ref, 'A02')">
         <xsl:value-of
            select="foo:indexName-Routine('work', false(), tokenize($biblstruct/monogr[1]/title[1]/@ref, ' ')[1], substring-after($biblstruct/monogr[1]/title[1]/@ref, ' '), false(), false(), $kommentar-oder-hrsg)"
         />
      </xsl:if>
      <xsl:if
         test="starts-with($biblstruct/analytic[1]/author[1]/@ref, 'A00') and not($biblstruct/analytic[1]/author[1]/@ref = 'A002003')">
         <xsl:value-of
            select="foo:indexName-Routine('person', false(), tokenize($biblstruct/analytic[1]/author[1]/@ref, ' ')[1], substring-after($biblstruct/analytic[1]/author[1]/@ref, ' '), false(), false(), $kommentar-oder-hrsg)"
         />
      </xsl:if>
      <xsl:if
         test="starts-with($biblstruct/monogr[1]/author[1]/@ref, 'A00') and not($biblstruct/analytic[1]/author[1]/@ref = 'A002003')">
         <xsl:value-of
            select="foo:indexName-Routine('person', false(), tokenize($biblstruct/monogr[1]/author[1]/@ref, ' ')[1], substring-after($biblstruct/monogr[1]/author[1]/@ref, ' '), false(), false(), $kommentar-oder-hrsg)"
         />
      </xsl:if>
      <xsl:choose>
         <!-- Zuerst Analytic -->
   ...
   <xsl:function name="foo:biographicalNote-rekursiv">
      <xsl:param name="biographicalNote" as="node()"/>
      <xsl:param name="counter" as="xs:integer"/>
      <xsl:variable name="anzahl" as="xs:integer" select="count($biographicalNote/note)"/>
      <xsl:choose>
      ...
      <xsl:choose>
         <xsl:when test="$biographicalNote/note[$counter]/bibl">
            <xsl:apply-templates select="$biographicalNote/note[$counter]/bibl"/>
            <xsl:text>: </xsl:text>
         </xsl:when>
         <xsl:when test="not($biographicalNote/note[$counter][bibl])">
            <xsl:text>\textcolor{red}{FEHLER QUELLENANGABE}</xsl:text>
         </xsl:when>
      </xsl:choose>
      <xsl:text>»</xsl:text>
      <xsl:apply-templates select="$biographicalNote/note[$counter]/quote"/>
      <xsl:text>«</xsl:text>
      <xsl:if
         test="not(substring(normalize-space($biographicalNote/note[$counter]/quote), string-length(normalize-space($biographicalNote/note[$counter]/quote)), 1) = '.' or substring(normalize-space($biographicalNote/note[$counter]/quote), string-length(normalize-space($biographicalNote/note[$counter]/quote)), 1) = '?' or substring(normalize-space($biographicalNote/note[$counter]/quote), string-length(normalize-space($biographicalNote/note[$counter]/quote)), 1) = '!' or $biographicalNote/note[$counter]/quote/node()[position() = last()]/self::dots or substring(normalize-space($biographicalNote/note[$counter]/quote), string-length(normalize-space($biographicalNote/note[$counter]/quote)) - 1, 2) = '.–')">
         <xsl:text>.</xsl:text>
      </xsl:if>
      <xsl:text>}</xsl:text>
      <xsl:if test="$anzahl &gt; $counter">
         <xsl:value-of select="foo:biographicalNote-rekursiv($biographicalNote, $counter + 1)"/>
      </xsl:if>
   </xsl:function>
   <!-- eigentlicher Fließtext root -->
   <xsl:template match="body">
      <xsl:param name="quellen" as="node()" select="ancestor::TEI/teiHeader/fileDesc/sourceDesc"/>
      <xsl:param name="gedruckte-quellen-count" as="xs:integer"
      ...
      <xsl:if
         ...
         <xsl:value-of
            select="foo:abgedruckte-workNameRoutine(substring(ancestor::TEI/teiHeader/fileDesc/titleStmt/title[@level = 'a']/@ref, 1, 7), substring-after(ancestor::TEI/teiHeader/fileDesc/titleStmt/title[@level = 'a']/@ref, ' '), true())"
         />
      </xsl:if>
...
-->
      ...
      <xsl:if test="$quellen/listBibl">
         <xsl:choose>
            ...
            <xsl:when test="ancestor::TEI[descendant::correspDesc/correspAction[@type='sent']/persName/@ref='#2121' and descendant::correspDesc/correspAction[@type='received']/persName/@ref='#10815']"></xsl:when>
-->
      ...
      <xsl:if test="child::div[@type = 'biographical']">
         <xsl:text>\toendnotes[C]{\smallbreak}</xsl:text>
         <xsl:value-of select="foo:biographicalNote-rekursiv(./div[@type = 'biographical'], 1)"/>
      </xsl:if>
      <xsl:choose>
      ...
      <xsl:if
         test="starts-with(ancestor::TEI/teiHeader/fileDesc/titleStmt/title[@level = 'a']/@ref, 'A0')">
         <xsl:value-of
            select="foo:abgedruckte-workNameRoutine(substring(ancestor::TEI/teiHeader/fileDesc/titleStmt/title[@level = 'a']/@ref, 1, 7), substring-after(ancestor::TEI/teiHeader/fileDesc/titleStmt/title[@level = 'a']/@ref, ' '), false())"
         />
      </xsl:if>
   ...
   <xsl:template match="body[parent::image]">
      ...
      <xsl:choose>
         ...
         <xsl:when test="ancestor::TEI/preceding-sibling::TEI[1]/body//descendant::textkonstitution"
            > \toendnotes[C]{\medbreak} </xsl:when>
         <xsl:otherwise> \toendnotes[C]{\smallbreak} </xsl:otherwise>
      </xsl:choose>
...
-->
      ...
      <xsl:choose>
         <xsl:when test="ancestor::TEI/teiHeader/profileDesc/correspDesc">
            <xsl:value-of
               select="foo:briefempfaenger-mehrere-persName-rekursiv(ancestor::TEI/teiHeader/profileDesc/correspDesc/correspAction[@type = 'received'], count(ancestor::TEI/teiHeader/profileDesc/correspDesc/correspAction[@type = 'received']/persName), ancestor::TEI/teiHeader/profileDesc/correspDesc/correspAction[@type = 'sent'], ancestor::TEI/teiHeader/profileDesc/correspDesc/correspAction[@type = 'sent']/date/@when, ancestor::TEI/teiHeader/profileDesc/correspDesc/correspAction[@type = 'sent']/date/@n, ancestor::TEI/teiHeader/profileDesc/correspDesc/correspAction[@type = 'sent']/date, false())"/>
            <xsl:value-of
               select="foo:briefsender-mehrere-persName-rekursiv(ancestor::TEI/teiHeader/profileDesc/correspDesc/correspAction[@type = 'sent'], count(ancestor::TEI/teiHeader/profileDesc/correspDesc/correspAction[@type = 'sent']/persName), ancestor::TEI/teiHeader/profileDesc/correspDesc/correspAction[@type = 'received'], ancestor::TEI/teiHeader/profileDesc/correspDesc/correspAction[@type = 'sent']/date/@when, ancestor::TEI/teiHeader/profileDesc/correspDesc/correspAction[@type = 'sent']/date/@n, ancestor::TEI/teiHeader/profileDesc/correspDesc/correspAction[@type = 'sent']/date, false())"
   ...
   <xsl:template match="body[ancestor::TEI[starts-with(@id, 'E')]]">
      <xsl:apply-templates/>
   </xsl:template>
   ...
   <xsl:template match="p">
      <xsl:if test="ancestor::quote[ancestor::physDesc] and not(position() = 1)">
         <xsl:text> / </xsl:text>
      </xsl:if>
      <xsl:apply-templates/>
   </xsl:template>
...
-->
      ...
      <xsl:choose>
         ...
         <xsl:otherwise>
            ...
            <xsl:if test="self::p and preceding-sibling::*[1] = preceding-sibling::head[1]">
               <xsl:text>\noindent{}</xsl:text>
            </xsl:if>
            <xsl:if test="parent::div[child::*[1]] = self::p">
               <xsl:text>\noindent{}</xsl:text>
            </xsl:if>
            <xsl:if
...
-\->      <xsl:text>{\\[\baselineskip]}</xsl:text>
   ...
   <xsl:template match="head">
      <xsl:choose>
         <xsl:when test="position() = 1 and parent::div/position() = 1"/>
         <xsl:when
            test="preceding-sibling::*[1][self::p[child::*[1][self::gap[@reason = 'outOfScope'] and not(child::*[2])]]]"
            >[\ldots]</xsl:when>
      ...
      <xsl:choose>
         <xsl:when
            test="following-sibling::*[1][self::p[child::*[1][self::gap[@reason = 'outOfScope'] and not(child::*[2])]]]"
            >[\ldots]</xsl:when>
         <xsl:when test="following-sibling::*[1][self::head[@type = 'sub']]">
   ...
   <xsl:template match="head[@type = 'sub']">
      ...
      <xsl:choose>
         <xsl:when test="following-sibling::*[1][self::head]"/>
         <xsl:otherwise>
            <xsl:text>\vspace{0.5\baselineskip}
         </xsl:text>
         </xsl:otherwise>
      ...
      <xsl:choose>
         ...
         <xsl:otherwise>
            <xsl:choose>
               ...
               <xsl:when
                  test="@scribe = '#2173' and ancestor::TEI/teiHeader[1]/fileDesc[1]/titleStmt[1]/author/@ref = '#2121'">
                  <xsl:value-of
               ...
               <!-- Wassermann: -->
               <xsl:when
                  test="@scribe = '#13058' and ancestor::TEI/teiHeader[1]/fileDesc[1]/titleStmt[1]/author/@ref = '#13055'">
                  <xsl:value-of
               ...
               <!-- Mary Mell -->
               <xsl:when
                  test="@scribe = '#5765' and ancestor::TEI/teiHeader[1]/fileDesc[1]/titleStmt[1]/author/@ref = '#12225'">
                  <xsl:value-of
                     select="normalize-space(key('person-lookup', (@scribe), $persons)/tei:persName/tei:forename)"
                  />
               </xsl:when>
               <xsl:when
                  test="@scribe = '#2292' and ancestor::TEI/teiHeader[1]/fileDesc[1]/titleStmt[1]/author/@ref = '#11740'">
                  <xsl:value-of
                     select="substring(normalize-space(key('person-lookup', (@scribe), $persons)/tei:persName/tei:forename), 1, 1)"/>
                  <xsl:text>. </xsl:text>
               </xsl:when>
               <xsl:when
                  test="@scribe = '#27886' and ancestor::TEI/teiHeader[1]/fileDesc[1]/titleStmt[1]/author/@ref = '#27882'">
                  <xsl:value-of
                     select="substring(normalize-space(key('person-lookup', (@scribe), $persons)/tei:persName/tei:forename), 1, 1)"/>
                  <xsl:text>. </xsl:text>
               </xsl:when>
               <xsl:when
                  test="@scribe = '#23918' and ancestor::TEI/teiHeader[1]/fileDesc[1]/titleStmt[1]/author/@ref = '#2167'">
                  <xsl:value-of
                     select="substring(normalize-space(key('person-lookup', (@scribe), $persons)/tei:persName/tei:forename), 1, 1)"/>
                  <xsl:text>. </xsl:text>
               </xsl:when>
   ...
   <!-- Großbuchstaben -->
   <xsl:template match="hi[@rend = 'capitals']//text()">
   ...
   <xsl:template match="foreign[@lang = 'el' or @xml:lang = 'el']">
      <xsl:text>\griechisch{</xsl:text>
      <xsl:apply-templates/>
      <xsl:text>}</xsl:text>
   </xsl:template>
   <xsl:template match="foreign[@lang = 'en' or @xml:lang = 'en']">
      <xsl:text>\begin{otherlanguage}{english}</xsl:text>
      <xsl:apply-templates/>
      <xsl:text>\end{otherlanguage}</xsl:text>
   </xsl:template>
   <xsl:template match="foreign[@lang = 'fr' or @xml:lang = 'fr']">
  ...
  Fließtext oder in Paratexten befindet und ob die Person namentlich genannt oder nur auf sie verwiesen wird -->
   ...
   <xsl:function name="foo:indexName-Routine">
      ...
      <xsl:choose>
         ...
         <xsl:when test="$typ = 'person'">
            <xsl:choose>
               ...
               <xsl:otherwise>
                  ...
                  <xsl:text>|pw</xsl:text>
                  <xsl:value-of
                     select="foo:indexeintrag-hinten($first, $verweis, $im-text, $certlow, $kommentar-oder-hrsg)"
                  />
               </xsl:otherwise>
            </xsl:choose>
         </xsl:when>
         <xsl:when test="$typ = 'work'">
            <!--  <xsl:value-of select="foo:werk-in-index($first, $im-text, false())"/>
             <xsl:text>|pw</xsl:text>
            <xsl:value-of select="foo:indexeintrag-hinten($first, $verweis, $im-text, $certlow, $kommentar-oder-hrsg)"/>-->
         </xsl:when>
         <xsl:when test="$typ = 'org'">
            <xsl:value-of select="foo:org-in-index($first, $im-text)"/>
            <xsl:text>|pw</xsl:text>
            <xsl:value-of
               select="foo:indexeintrag-hinten($first, $verweis, $im-text, $certlow, $kommentar-oder-hrsg)"
            />
      ...
      <xsl:if test="$rest != ''">
         <xsl:value-of
            select="foo:indexName-Routine($typ, $verweis, tokenize($rest, ' ')[1], substring-after($rest, ' '), $im-text, $certlow, $kommentar-oder-hrsg)"
         />
      </xsl:if>
   </xsl:function>
   <xsl:template match="persName | workName | orgName | placeName | rs">
      <xsl:variable name="inhalt" select="string(.)"/>
      <xsl:variable name="first" select="tokenize(@ref, ' ')[1]"/>
      ...
      <xsl:variable name="candidate" as="xs:boolean"
         select="ancestor::TEI/teiHeader/revisionDesc/@status = 'approved' or ancestor::TEI/teiHeader/revisionDesc/@status = 'candidate'"/>
      ...
      <xsl:variable name="kommentar-herausgeber" as="xs:boolean"
         select="(ancestor::note[@type = 'commentary'] or ancestor::note[@type = 'textConst'] or ancestor::TEI[starts-with(@id, 'E')] or ancestor::physDesc) and not(parent::quote)"/>
      ...
      <xsl:choose>
         <xsl:when test="$first = '' or empty($first)">
            <xsl:choose>
               <xsl:when
                  test="(ancestor::bibl or ancestor::biblStruct) and not($verweis) and @type = 'work'">
                  <xsl:text>\emph{</xsl:text>
                  <xsl:choose>
                     <xsl:when test="not(child::*[1]) and ancestor::hi[@rend = 'capitals']">
               ...
               <xsl:when test="(ancestor::bibl or ancestor::biblStruct)">
                  <xsl:choose>
                     <xsl:when test="not(child::*[1]) and ancestor::hi[@rend = 'capitals']">
                        <xsl:value-of select="fn:upper-case(.)"/>
                     </xsl:when>
         ...
         <xsl:otherwise>
            <xsl:choose>
               <xsl:when
                  test="@type = 'work' and $kommentar-herausgeber and not($verweis) and not(ancestor::quote)">
                  <xsl:text>\emph{</xsl:text>
                  <xsl:choose>
                     <xsl:when test="not(child::*[1]) and ancestor::hi[@rend = 'capitals']">
                        <xsl:value-of select="fn:upper-case(.)"/>
                  ...
                  <xsl:text>}</xsl:text>
                  <xsl:value-of
                     select="foo:indexName-Routine(@type, $verweis, tokenize(@ref, ' ')[1], substring-after(@ref, ' '), $im-text, $cert, $kommentar-herausgeber)"
                  />
               </xsl:when>
               <xsl:when
                  test="@type = 'work' and $kommentar-herausgeber and $verweis and not(ancestor::quote)">
                  <xsl:choose>
                     <xsl:when test="not(child::*[1]) and ancestor::hi[@rend = 'capitals']">
                        <xsl:value-of select="fn:upper-case(.)"/>
                     </xsl:when>
               ...
               <xsl:when test="@type = 'org' and $kommentar-herausgeber and not(parent::bibl)">
               ...
               <xsl:when test="@type = 'org' and $kommentar-herausgeber and parent::bibl">
                  <xsl:if test="not(@subtype='implied')">
                  <xsl:text>\emph{</xsl:text>
                  </xsl:if>
                  <xsl:apply-templates/>
               ...
               <xsl:when test="$im-text and $candidate">
                  <xsl:choose>
                     <xsl:when test="not(child::*[1]) and ancestor::hi[@rend = 'capitals']">
                        <xsl:value-of select="fn:upper-case(.)"/>
                     </xsl:when>
               ...
               <xsl:when test="$index-test-bestanden and not($candidate) and $im-text">
   ...
   <xsl:function name="foo:werkInEndnote">
      ...
      <xsl:if test="$entry/tei:author[1]/tei:surname/text() != ''">
         <xsl:for-each select="$entry/tei:author">
            <xsl:choose>
   ...
   <xsl:function name="foo:orgNameRoutine">
      ...
      <xsl:if test="$first != ''">
         <xsl:value-of select="foo:org-in-index($first, $im-text)"/>
         <xsl:choose>
            <xsl:when test="empty(key('org-lookup', $first, $orgs)/tei:orgName)"/>
            <xsl:otherwise>
               <xsl:text>|pw</xsl:text>
               <xsl:choose>
                  <xsl:when test="$certlow = true()">
         ...
         <xsl:if test="$rest != ''">
            <xsl:value-of
               select="foo:orgNameRoutine(substring($rest, 1, 7), substring-after($rest, ' '), $verweis, $im-text, $certlow, $kommentar-oder-hrsg)"
            />
         </xsl:if>
      </xsl:if>
   </xsl:function>
   ...
   <xsl:function name="foo:placeNameRoutine">
      ...
      <xsl:if test="$rest != ''">
         <xsl:value-of
            select="foo:placeNameRoutine(substring($rest, 1, 7), substring-after($rest, ' '), $verweis, $im-text, $certlow, $kommentar-oder-hrsg)"
         />
      </xsl:if>
   </xsl:function>
   ...
   <xsl:function name="foo:place-in-index">
      ...
      <xsl:variable name="typ" select="$place/desc"/>
      <xsl:choose>
         <xsl:when test="not(starts-with($first, '#'))">
   ...
   <xsl:template match="figure">
      ...
      <!-- Zwei Varianten. Wenn das Bild max. bis zur halben Textbreite geht, wird die Bildunterschrift daneben gesetzt = Variante 1 -->
      <xsl:choose>
         <xsl:when test="$numbers &lt; 7">
            <xsl:choose>
               <xsl:when
                  test="ancestor::TEI//teiHeader[1]/fileDesc[1]/publicationStmt[1]/idno[1]/@type = 'ASI-E'">
   ...
   <xsl:template match="caption">
      ...
      <xsl:text>\begin{flushleft}\footnotesize\emph{</xsl:text>
      <xsl:apply-templates/>
      <xsl:text>}\end{flushleft}\end{minipage}\vspace{\baselineskip}
      </xsl:text>
   </xsl:template>
