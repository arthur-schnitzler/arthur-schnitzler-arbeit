    ...
    <xsl:template match="tei:correspDesc">
        <xsl:element name="correspDesc" namespace="http://www.tei-c.org/ns/1.0">
            ...
            <xsl:if test="key('kontext-lookup', ancestor::tei:TEI/@xml:id, $schnitzler-briefe_cmif)">
                <xsl:copy-of select="key('kontext-lookup', ancestor::tei:TEI/@xml:id, $schnitzler-briefe_cmif)/tei:correspContext" exclude-result-prefixes="#all" copy-namespaces="false"/>
            </xsl:if>
